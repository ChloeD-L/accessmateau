type EventHandlerCallback<P extends any = void, V extends any = void> = (val: P) => V;
...
export class EventManager<E extends string = string> {
  ...
  once(event: E, handler: EventHandlerCallback) {
    ...
    const onceHandler = () => {
      handler();
      this.handlersByEvent[event] = this.handlersByEvent[event].filter((h) => h !== onceHandler);
    };
  ...
  off(event: E, handler?: EventHandlerCallback) {
    ...
    this.handlersByEvent[event] = this.handlersByEvent[event].filter((h) => h !== handler);
  }
  emit(event: E) {
    if (!this.handlersByEvent[event]) {
      return;
...
export type FieldType<T, P extends Path<T>> = P extends `${infer K}.${infer Rest}`
  ? K extends keyof T
